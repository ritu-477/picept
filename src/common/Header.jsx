                        ...
                        <div className={`menuList ${isMenuOpen ? 'max-lg:left-0' : 'max-lg:left-[-100%]'} z-10 gap-6 max-lg:w-full max-lg:fixed max-lg:justify-center max-lg:top-0 max-lg:bg-black max-lg:transition-all duration-300 flex items-center max-lg:min-h-screen`} ref={menuRef}>
                            <ul className='flex flex-col lg:flex-row gap-6 items-center lg:items-center z-10'>
                                <li className="relative">
                                    <a onClick={toggleDropdown} className='font-bold gap-1 relative text-primary-white lg:text-custom2xl lg:leading-7 text-lg flex items-center space-x-1 cursor-pointer after:absolute hover:after:bg-white hover:after:left-0 after:w-0 hover:after:w-full hover:after:h-[2px] after:left-0 hover:after:bottom-[-3px] after:ease-in-out after:duration-300' href="#">Product
                                        <span>
                                ...
                                {NAV_DATA_LINK.map((link, index) => (
                                    <li key={index} onClick={toggleMenu}>
                                        <a target='blank' className='font-bold text-primary-white lg:text-custom2xl lg:leading-7 text-lg relative after:absolute hover:after:bg-white hover:after:left-0 after:w-0 hover:after:w-full hover:after:h-[1px] after:left-0 hover:after:bottom-[-3px] after:ease-in-out after:duration-300' href={link.href}>
                                            {link.label}</a>
                                    </li>
                                ))}
                                <a href="#_" className='rounded-full border hover:bg-white hover:text-ink-black duration-700 border-primary-white py-3 bg-ink-black px-7 lg:hidden block overflow-hidden relative group cursor-pointer text-base font-medium text-primary-white z-10'>Docs</a>
                                <a href="#_" className='rounded-full py-3 bg-orange hover:bg-white hover:text-orange duration-700 px-7 lg:hidden block overflow-hidden relative group cursor-pointer text-base font-medium text-primary-white z-10'>Book a Demo</a>
                            </ul>
                        </div>
                    </div>
